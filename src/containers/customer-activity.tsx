...
import type { CustomerActivity as CustomerActivityInterface } from '../types/customer';

interface ActivitiesState {
  data?: CustomerActivityInterface[];
  error?: string;
  isLoading: boolean;
}

export const CustomerActivity: FC = () => {
  ...
  const displayError = Boolean(
    !activitiesState.isLoading && activitiesState.error
  );
        ...
        >
          <List disablePadding>
            {activitiesState.data?.map((activity) => (
              <CustomerActivityItem divider key={activity.id} {...activity} />
            ))}
          </List>
          {displayLoading && (
            <Box sx={{ p: 2 }}>
          ...
          {displayError && (
            <ResourceError error={activitiesState.error} sx={{ m: 2 }} />
          )}
